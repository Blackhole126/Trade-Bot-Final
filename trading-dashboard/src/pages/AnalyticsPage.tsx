...
const AnalyticsPage = () => {
  ...
  // Load symbols from user-scoped storage on mount
  useEffect(() => {
    if (!user?.username) return;
    try {
      const stored = userStorage.getItem('analytics_symbols');
      if (stored) {
  ...
  // Persist symbols to user-scoped storage
  useEffect(() => {
    if (!user?.username) return;
    userStorage.setItem('analytics_symbols', JSON.stringify(analyticsSymbols));
  }, [analyticsSymbols, user?.username]);
