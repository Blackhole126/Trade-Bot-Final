...
const DashboardPage = () => {
  ...
  // Load user-added trades and hidden trades from user-scoped storage on mount
  useEffect(() => {
    if (!user?.username) return;

    const savedTrades = userStorage.getItem('userAddedTrades');
    if (savedTrades) {
      try {
    ...
    const savedHidden = userStorage.getItem('hiddenTrades');
    if (savedHidden) {
      try {
  ...
  // Tab visibility change listener - refresh data when tab becomes visible
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible' && connectionState.isConnected) {
        console.log('[Dashboard] Tab became visible, data will be refreshed on next interval');
        // Note: We don't immediately refresh here to avoid rapid state changes
        // The regular refresh interval will pick up fresh data
      }
    };
  ...
  // Save user-added trades to user-scoped storage
  // Clear all cached trades and reload
  const clearCacheAndReload = () => {
    userStorage.removeItem('userAddedTrades');
    userStorage.removeItem('visibleTopStocks');
    setUserAddedTrades([]);
    setTopStocks([]);
  ...
  const saveUserAddedTrades = (trades: PredictionItem[]) => {
    ...
    userStorage.setItem('userAddedTrades', JSON.stringify(trades));
  };

  // Save hidden trades to user-scoped storage
  const saveHiddenTrades = (symbols: string[]) => {
    setHiddenTrades(symbols);
    userStorage.setItem('hiddenTrades', JSON.stringify(symbols));
  };
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-700/30">
                                <div>
                                  ...
                                  <p className={`text-xs font-bold ${dataIntegrity?.confidence === 'HIGH' ? 'text-green-400' :
                        ...
                        <div className="flex items-center gap-4 flex-shrink-0">
                          ...
                          <div className="text-right border-l border-slate-700/50 pl-4 min-w-[70px]">
                            ...
                            <div className="flex items-center gap-1.5 justify-end">
                              <div className={`w-1.5 h-1.5 rounded-full ${dataIntegrity?.confidence === 'HIGH' ? 'bg-green-400' :
                                  dataIntegrity?.confidence === 'MEDIUM' ? 'bg-yellow-400' :
                                    'bg-red-400'
                                }`}></div>
                              <p className={`text-xs font-semibold ${dataIntegrity?.confidence === 'HIGH' ? 'text-green-400' :
                                  dataIntegrity?.confidence === 'MEDIUM' ? 'text-yellow-400' :
                                    'text-red-400'
                                }`}>
                                {dataIntegrity?.confidence || 'UNKNOWN'}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {['RELIANCE.NS', 'TCS.NS', 'HDFCBANK.NS', 'INFY.NS', 'SBIN.NS', 'TATAMOTORS.NS'].map((symbol) => (
              <button
                ...
                className={`px-2 py-1.5 text-xs rounded-lg transition-all ${symbolExistsInTopPerformers(symbol)
