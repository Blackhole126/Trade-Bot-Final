...
import type { HftBotData, HftLiveStatus } from '../../types/hft';
...
const StatusDot = styled.div<{ mode: string; $connected: boolean }>`
  width: 8px;
  height: 8px;
  border-radius: 50%;
  background: ${props => {
...
const BotStatusDot = styled.div<{ $active: boolean }>`
  width: 10px;
  height: 10px;
  border-radius: 50%;
...
const SettingsButton = styled.button<{ disabled: boolean }>`
...
interface HftHeaderProps {
    ...
    onOpenSettings: () => void;
    liveStatus: HftLiveStatus | null;
}

const HftHeader: React.FC<HftHeaderProps> = ({
    botData,
    activeTab,
    onTabChange,
    onOpenSettings,
