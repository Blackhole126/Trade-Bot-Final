...
class MCPAdapter:
    ...
    news_data: Optional[List[Dict]] = None
        ...
        try:
           ...
           # 🔥 FINAL NORMALIZATION (PHASE 2 FIX)
            normalized_predictions = []

            for p in predictions:
                normalized_predictions.append({
                "symbol": p.get("symbol"),
                "status": "error" if p.get("error") else "success",
            ...
            # Replace predictions with normalized version
            response["predictions"] = normalized_predictions
