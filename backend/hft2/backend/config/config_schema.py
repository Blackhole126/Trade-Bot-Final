...
class ConfigValidator:
    ...
    def validate_environment_variables() -> List[str]:
        ...
        NOTE: DHAN_CLIENT_ID and DHAN_ACCESS_TOKEN are NOT checked here —
        they are fetched per-user from MongoDB at runtime, not from server env vars.
        """
        issues = []
...
def load_and_validate_config(mode: str = "paper", username: Optional[str] = None) -> Dict[str, Any]:
    """
    Load configuration from file and validate against schema

    Args:
        mode: Trading mode ("paper" or "live")
        username: Specific user context for configuration loading
    ...
    try:
        ...
        raw_config = load_config_from_file(mode, username=username)
...
def load_config_from_file(mode: str, username: Optional[str] = None) -> Dict[str, Any]:
    """Load configuration from the appropriate JSON file, optionally session-isolated."""
    try:
        import os
        import json

        # Get the data directory path (backend/hft2/data)
        ...
        if username and username != "anonymous":
            ...
            safe_un = "".join(c for c in str(username) if c.isalnum() or c in ("-", "_")).strip() or "anonymous"
            data_dir = os.path.join(data_dir, "users", safe_un)
        ...
        if os.path.exists(config_file):
            with open(config_file, 'r') as f:
                ...
                logger.info(f"Loaded configuration from {config_file} for {username}")
                return config_data
        else:
            # MULTI-USER FIX: Never fall back to the global (username=None) config.
            # That causes session leakage where a new user without a config file
            # sees the "anonymous" user's data (leaked from a previous session).
            logger.info(f"Config file {config_file} not found for {username}, using defaults")
            return {}
