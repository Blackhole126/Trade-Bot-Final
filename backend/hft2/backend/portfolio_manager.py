...
class DualPortfolioManager:
    ...
    def __init__(self, data_dir: str = "data", user_id: Optional[str] = None):
        # Prefer DATA_DIR env var (set to /tmp/data on Render); fall back to project_root/data
        ...
        # PRODUCTION FIX: Use user-specific data directory for isolation if user_id is set
        if self.user_id and self.user_id != "anonymous":
            self.data_dir = os.path.join(self.data_dir, "users", self.user_id)
            os.makedirs(self.data_dir, exist_ok=True)
            logger.info(f"DualPortfolioManager using isolated data directory: {self.data_dir}")
        self.current_mode = "live"  # Default mode
        ...
        try:
            ...
            logger.info(
                f"Dual Portfolio Manager initialized with SQLite database in {data_dir}")
        ...
        try:
            ...
            for mode in ["live"]:
                ...
                if not portfolio:
                    ...
                    json_file = self._get_portfolio_json_path(mode)
    ...
    def _get_portfolio_json_path(self, mode: str) -> str:
        """Get user-specific portfolio JSON path."""
        # Note: self.data_dir is already user-specific if user_id was provided
        return os.path.join(self.data_dir, f'portfolio_india_{mode}.json')

    def _get_trade_log_json_path(self, mode: str) -> str:
        """Get user-specific trade log JSON path."""
        return os.path.join(self.data_dir, f'trade_log_india_{mode}.json')
    ...
    def load_mode(self, mode: str) -> None:
        ...
        try:
            ...
            logger.info(
                f"Number of holdings in database: {len(portfolio.holdings)}")
            for holding in portfolio.holdings:
                logger.info(
                    f"Processing holding: {holding.ticker}, quantity: {holding.quantity}")
    ...
    def _update_data_service_watchlist_after_trade(self, session, portfolio):
        ...
        try:
            ...
            logger.info(
                f"Updated data service watchlist after trade with {len(holding_symbols)} symbols")

        except Exception as e:
            logger.error(
                f"Failed to update data service watchlist after trade: {e}")
    ...
    def _sync_to_json(self, session, portfolio):
        ...
        try:
            ...
            # Save to JSON
            json_file = self._get_portfolio_json_path(portfolio.mode)
            ...
            trades_file = self._get_trade_log_json_path(portfolio.mode)
    ...
    def get_portfolio_summary(self) -> Dict:
        ...
        try:
            ...
            holdings_value = sum(holding.quantity *
                                 holding.last_price for holding in holdings)

            # Calculate unrealized P&L
            cost_basis = sum(holding.quantity *
                             holding.avg_price for holding in holdings)
            ...
            return_percentage = (total_return / self.current_portfolio.starting_balance *
                                 100) if self.current_portfolio.starting_balance > 0 else 0
    ...
    def update_cash_balance(self, new_cash: float) -> bool:
        ...
        try:
            ...
            if portfolio:
                ...
                session.commit()
                logger.info(
                    f"Updated cash balance to ₹{new_cash:.2f} for {self.current_mode} portfolio")
