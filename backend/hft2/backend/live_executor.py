...
class LiveTradingExecutor:
    ...
    def __init__(self, portfolio_manager: DualPortfolioManager, config: Dict):
        ...
        # Initialize Dhan client — credentials MUST come from config (injected from MongoDB by _initialize_live_trading)
        # env-var fallback deliberately removed: all credentials are dynamic per-user from MongoDB
        dhan_client_id = config.get("dhan_client_id")
        dhan_access_token = config.get("dhan_access_token")

        if not dhan_client_id or not dhan_access_token:
            logger.error(
                "Dhan credentials missing from config. "
                "Go to Settings → Demat account → Link your demat to save your Client ID & Access Token."
            )
            raise ValueError(
                "Dhan credentials required for live trading. Link your demat account in Settings first."
            )
        ...
        logger.info(
            "Skipping portfolio sync during initialization (will sync lazily)")
    ...
    def refresh_dynamic_config(self):
        """Refresh dynamic configuration from the latest config"""
        old_stop_loss = self.stop_loss_pct
        old_allocation = self.max_capital_per_trade
        ...
        # Log changes
        if old_stop_loss != self.stop_loss_pct or old_allocation != self.max_capital_per_trade:
            logger.info(f"🔄 Executor config refreshed:")
            logger.info(
                f"   Stop Loss: {self.stop_loss_pct:.1%} (was {old_stop_loss:.1%})")
            logger.info(
                f"   Max Allocation: {self.max_capital_per_trade:.1%} (was {old_allocation:.1%})")
            logger.info(f"   Max Trade Limit: {self.max_trade_limit}")
        ...
        try:
            ...
            try:
                ...
                portfolio = session.query(Portfolio).filter_by(
                    **self._live_portfolio_filter()).first()
                if not portfolio:
                    portfolio = Portfolio(
                        mode='live',
                        user_id=getattr(self.portfolio_manager,
                                        "user_id", None),
                        cash=available_cash,
    ...
    def calculate_position_size(self, symbol: str, current_price: float,
                                signal_strength: float, is_short: bool = False) -> int:
        """
        ...
        Args:
            is_short: If True, this is for short-selling (use more conservative sizing)
        """
        session = None
        try:
            session = self.portfolio_manager.db.Session()
            ...
            # For short-selling, use more conservative position sizing (50% of normal size)
            if is_short:
                max_capital_per_trade *= 0.50  # Use only 50% for shorts
                logger.info(f"🔴 Short-selling: Reducing position size to 50% ({max_capital_per_trade:.1%} of cash)")
    ...
    def execute_buy_order(self, symbol: str, signal_data: Dict) -> Dict:
        ...
        try:
            ...
            # Place market buy order via Dhan API
            logger.info(
                f"🚀 PLACING LIVE DHAN ORDER: BUY {adjusted_quantity} {symbol} @ Rs.{current_price:.2f}")
            
            # Get product type from config (default to CNC for delivery)
            product_type = self.config.get("product_type", "CNC")
            ...
            order_response = self.dhan_client.place_order(
                ...
                order_type="MARKET",
                side="BUY",
                product_type=product_type  # Required by Dhan API
            )

            # CRITICAL: Verify order response
            if not order_response:
                logger.error(
                    "❌ DHAN ORDER FAILED: Empty response from Dhan API")
            ...
            if isinstance(order_response, dict) and order_response.get("status") == "failure":
                error_type = order_response.get("errorType", "Unknown")
                error_code = order_response.get("errorCode", "DH-905")
                error_msg = order_response.get("errorMessage", "Unknown error")
                logger.error(
                    f"❌ DHAN ORDER REJECTED: {error_type} ({error_code}): {error_msg}")
                return {"success": False, "message": f"Dhan order rejected: {error_msg}"}

            order_id = order_response.get(
                "orderId") or order_response.get("order_id")
            if order_id:
                logger.info(
                    f"✅ DHAN ORDER CONFIRMED: Order ID {order_id}")
                # Track pending order
                ...
                try:
                    ...
                    logger.info(
                        f"✅ Buy trade recorded in database: {adjusted_quantity} {symbol} at Rs.{current_price:.2f}")
                except Exception as db_error:
                    logger.error(
                ...
                logger.info(
                    f"🎯 LIVE ORDER EXECUTED ON DHAN: {adjusted_quantity} {symbol} @ Rs.{current_price:.2f}")
        ...
        try:
            ...
            try:
                ...
                if signal_data and signal_data.get("quantity") is not None:
                    sell_qty = min(
                        int(signal_data["quantity"]), holding.quantity)
                if sell_qty <= 0:
                ...
                logger.info(
                    f"🚀 PLACING LIVE DHAN ORDER: SELL {sell_qty} {symbol} @ Rs.{current_price:.2f}")
                ...
                if order_id:
                    ...
                    try:
                        ...
                        logger.info(
                            f"✅ Sell trade recorded in database: {sell_qty} {symbol} at Rs.{current_price:.2f}, P&L: Rs.{pnl:.2f}")
                    except Exception as db_error:
                        logger.error(
                            f"❌ Failed to record sell trade in database: {db_error}")

                    logger.info(
                        f"🎯 LIVE SELL ORDER EXECUTED ON DHAN: {sell_qty} {symbol} @ Rs.{current_price:.2f}")
                    logger.info(f"   Order ID: {order_id}")
                    logger.info(
                        f"   Check your Dhan app/web terminal to confirm order status")
                    logger.info(
    ...
    def execute_short_sell_order(self, symbol: str, signal_data: Dict) -> Dict:
        """
        Execute a SHORT-SELL order (sell first without holdings, then buy later to square off)
        This is ONLY allowed for MIS (intraday) product type.
        
        For short-selling:
        - We SELL first at current price (opening a short position)
        - Stop-loss is ABOVE entry price (price going up = loss)
        - Take-profit is BELOW entry price (price going down = profit)
        - Must square off by BUYING before market closes (3:15 PM auto square-off)
        """
        try:
            logger.info(f"🔴 SHORT-SELL EXECUTION REQUESTED for {symbol}")
            
            # CRITICAL: Verify this is MIS (intraday) product type
            product_type = self.config.get("product_type", "MIS")
            if product_type != "MIS":
                logger.error(f"❌ Short-selling only allowed for MIS (intraday) product type, not {product_type}")
                return {"success": False, "message": "Short-selling only allowed for MIS (intraday) product type. Please switch product type in settings."}
            
            # Check if short-selling is enabled
            enable_short_selling = self.config.get("enable_short_selling", True)
            if not enable_short_selling:
                logger.info("Short-selling disabled by configuration. Skipping.")
                return {"success": False, "message": "Short-selling disabled by configuration"}
            
            if not self.enable_sell:
                logger.info("Sell disabled by configuration. Skipping short-sell.")
                return {"success": False, "message": "Sell disabled by configuration"}
            ...
            if current_price <= 0:
                ...
                logger.info(f"Using signal price for {symbol}: Rs.{current_price:.2f}")
            
            # Get quantity from signal data or calculate position size
            requested_quantity = signal_data.get("quantity", 0)
            if requested_quantity <= 0:
                signal_strength = signal_data.get("confidence", 0.5)
                requested_quantity = self.calculate_position_size(
                    symbol, current_price, signal_strength, is_short=True)
            
            if requested_quantity <= 0:
                return {"success": False, "message": "Insufficient margin or invalid quantity"}
            
            # For short-selling, we need to check margin requirement (not cash)
            # Dhan provides 5x leverage for MIS intraday
            margin_required = (current_price * requested_quantity) * 0.20  # 20% margin for shorts
            
            # Adjust quantity based on available margin
            adjusted_quantity = self._adjust_quantity_for_short_margin(
                symbol, current_price, requested_quantity, margin_required)
            
            if adjusted_quantity <= 0:
                return {"success": False, "message": "Insufficient margin for short position"}
            
            # Get stop-loss and take-profit from signal data
            stop_loss = signal_data.get("stop_loss", current_price * 1.03)  # 3% above by default
            take_profit = signal_data.get("take_profit", current_price * 0.97)  # 3% below by default
            
            # Validate short levels: SL must be ABOVE entry, TP must be BELOW entry
            if stop_loss <= current_price:
                logger.warning(f"Stop-loss {stop_loss:.2f} is not above entry {current_price:.2f}. Adjusting...")
                stop_loss = current_price * 1.03
            
            if take_profit >= current_price:
                logger.warning(f"Take-profit {take_profit:.2f} is not below entry {current_price:.2f}. Adjusting...")
                take_profit = current_price * 0.97
            
            logger.info(f"🚀 PLACING LIVE SHORT-SELL ORDER: SELL {adjusted_quantity} {symbol} @ Rs.{current_price:.2f}")
            logger.info(f"   Product Type: MIS (Intraday)")
            logger.info(f"   Stop-Loss: Rs.{stop_loss:.2f} (ABOVE entry)")
            logger.info(f"   Take-Profit: Rs.{take_profit:.2f} (BELOW entry)")
            
            # Place SHORT-SELL order via Dhan API with is_short_sell flag
            order_response = self.dhan_client.place_order(
                symbol=symbol,
                side="SELL",
                quantity=adjusted_quantity,
                order_type="MARKET",
                product_type="MIS",  # Always MIS for short-selling
                is_short_sell=True  # Flag this as a short-sell order
            )
            
            # Verify order response
            if not order_response:
                logger.error("❌ SHORT-SELL ORDER FAILED: Empty response from Dhan API")
                return {"success": False, "message": "Dhan API returned empty response"}
            
            if isinstance(order_response, dict) and order_response.get("status") == "failure":
                error_msg = order_response.get("errorMessage", "Unknown error")
                logger.error(f"❌ SHORT-SELL ORDER REJECTED: {error_msg}")
                return {"success": False, "message": f"Short-sell rejected: {error_msg}"}
            
            order_id = order_response.get("orderId") or order_response.get("order_id")
            if order_id:
                logger.info(f"✅ SHORT-SELL ORDER CONFIRMED: Order ID {order_id}")
                
                # Track pending short position
                self.pending_orders[order_id] = {
                    "symbol": symbol,
                    "quantity": adjusted_quantity,
                    "side": "SHORT_SELL",  # Mark as short sell
                    "price": current_price,
                    "stop_loss": stop_loss,
                ...
                try:
                    self.portfolio_manager.record_trade(
                        ticker=symbol,
                        action="sell",  # Still "sell" but marked as short in metadata
                        quantity=adjusted_quantity,
                        price=current_price,
                        stop_loss=stop_loss,
                        take_profit=take_profit,
                        metadata={
                            ...
                            "square_off_required": True,
                            "square_off_by": "BUY"
                        }
                    )
                    logger.info(f"✅ Short-sell recorded: {adjusted_quantity} {symbol} at Rs.{current_price:.2f}")
                except Exception as db_error:
                ...
                return {
                    "success": True,
                    "message": f"Short-sell executed: {adjusted_quantity} {symbol} at Rs.{current_price:.2f}",
                    "order_id": order_id,
                    "quantity": adjusted_quantity,
                    "price": current_price,
                    "stop_loss": stop_loss,
                ...
                return {"success": False, "message": "Invalid order response from Dhan"}
        
        except Exception as e:
            logger.error(f"❌ Failed to execute short-sell order for {symbol}: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return {"success": False, "message": f"Short-sell failed: {str(e)}"}
    
    def _adjust_quantity_for_short_margin(
        self,
        symbol: str,
        current_price: float,
        ...
        """
        Adjust short-sell quantity based on available margin
        For MIS intraday shorts, Dhan requires ~20% margin
        """
        try:
            # Get available cash/margin from portfolio
            session = None
            try:
                session = self.portfolio_manager.db.Session()
                portfolio = session.query(Portfolio).filter_by(
                ...
                if not portfolio:
                    logger.warning("No live portfolio found for margin check")
                    return requested_quantity
                
                available_cash = float(portfolio.cash or 0.0)
                
                # For conservative risk management, use only 50% of available cash for any single short
                max_margin_per_trade = available_cash * 0.50
                
                logger.info(f"Available cash: Rs.{available_cash:.2f}, Max margin for short: Rs.{max_margin_per_trade:.2f}")
                
                # Calculate maximum quantity we can short with available margin
                if margin_required > max_margin_per_trade:
                    # Reduce quantity proportionally
                    max_affordable_qty = int((max_margin_per_trade / margin_required) * requested_quantity)
                    logger.warning(f"Margin constraint: Reducing quantity from {requested_quantity} to {max_affordable_qty}")
                    return max(1, max_affordable_qty)  # Ensure at least 1 share
            ...
            logger.error(f"Error adjusting quantity for short margin: {e}")
            return requested_quantity
