...
class SellSignal:
    ...
    reasoning: str
    confidence: float
    # Category of the signal (Technical, Risk, Sentiment, ML, Market)
    category: str = ""
...
class ProfessionalSellLogic:
    ...
    def __init__(self, config: Dict):
        ...
        self.min_signals_required = config.get(
            "min_sell_signals", 2)  # Minimum 2 categories
        # 45% minimum confidence (moderate-strict)
        self.min_confidence_threshold = config.get("min_sell_confidence", 0.45)
        # 6% minimum weighted score (moderate-strict)
        self.min_weighted_score = config.get("min_weighted_sell_score", 0.06)

        # Stop-loss configuration
        self.base_stop_loss_pct = config.get("stop_loss_pct", 0.05)
        # 65% of stop-loss (moderate-strict)
        self.trailing_stop_pct = config.get("trailing_stop_pct", 0.0325)
        self.profit_protection_threshold = config.get(
            # 6% profit lock (moderate-strict)
            "profit_protection_threshold", 0.06)

        # Emergency loss threshold (8-10% loss triggers immediate sell)
        self.emergency_loss_threshold = config.get(
            "emergency_loss_threshold", 0.10)  # 10% default

        # Position sizing - More granular thresholds
        self.conservative_exit_threshold = config.get(
            "conservative_exit_threshold", 0.15)
        self.partial_exit_threshold = config.get(
            "partial_exit_threshold", 0.30)
        self.aggressive_exit_threshold = config.get(
            "aggressive_exit_threshold", 0.50)
        self.full_exit_threshold = config.get("full_exit_threshold", 0.70)
        self.emergency_exit_threshold = config.get(
            "emergency_exit_threshold", 0.90)

        # Market context filters - Less restrictive
        # Less restriction in uptrends (moderate-strict)
        self.uptrend_sell_multiplier = config.get(
            "uptrend_sell_multiplier", 1.15)
        # Less restriction in downtrends (moderate-strict)
        self.downtrend_sell_multiplier = config.get(
            "downtrend_sell_multiplier", 0.85)

        logger.info(
            "Professional Sell Logic initialized with institutional parameters")
    ...
    def _load_dynamic_config(self):
        ...
        try:
            ...
            if os.path.exists(config_path):
                ...
                self.base_stop_loss_pct = live_config.get(
                    "stop_loss_pct", self.base_stop_loss_pct)
                self.trailing_stop_pct = live_config.get(
                    "trailing_stop_pct", self.trailing_stop_pct)
                self.profit_protection_threshold = live_config.get(
                    "profit_protection_threshold", self.profit_protection_threshold)
                self.emergency_loss_threshold = live_config.get(
                    "emergency_loss_threshold", self.emergency_loss_threshold)

                logger.info(
                    f"📊 Loaded dynamic config for sell logic - Stop Loss: {self.base_stop_loss_pct:.1%}")
            else:
                logger.warning(
                    "live_config.json not found for sell logic, using defaults")
    ...
    def refresh_dynamic_config(self):
        ...
        old_max_loss = self.max_loss_threshold
        old_time_decay = self.time_decay_threshold

        self._load_dynamic_config()

        # Log if config changed
        if old_max_loss != self.max_loss_threshold or old_time_decay != self.time_decay_threshold:
            logger.info(f"🔄 Sell logic config refreshed:")
            logger.info(
                f"   Max Loss Threshold: {self.max_loss_threshold:.1%} (was {old_max_loss:.1%})")
            logger.info(
                f"   Time Decay Threshold: {self.time_decay_threshold} days (was {old_time_decay} days)")
    ...
    def evaluate_sell_decision(
        ...
        logger.info(
            f"Unrealized P&L: {position_metrics.unrealized_pnl:.2f} ({position_metrics.unrealized_pnl_pct:.2%})")
        logger.info(
        ...
        if position_metrics.db_stop_loss is not None and position_metrics.current_price <= position_metrics.db_stop_loss:
            ...
            logger.info(
                f"✅ Stored stop-loss triggered: {position_metrics.current_price:.2f} <= {position_metrics.db_stop_loss:.2f}")
            return SellDecision(
                should_sell=True,
        ...
        if position_metrics.db_target_price is not None and position_metrics.current_price >= position_metrics.db_target_price:
            ...
            logger.info(
                f"✅ Stored take-profit triggered: {position_metrics.current_price:.2f} >= {position_metrics.db_target_price:.2f}")
            return SellDecision(
                should_sell=True,
                sell_quantity=position_metrics.quantity,
                sell_percentage=1.0,
        ...
        technical_signals = self._generate_technical_signals(
            technical_analysis, position_metrics)
        ...
        # Combine all signals
        all_signals = technical_signals + risk_signals + \
            sentiment_signals + ml_signals + market_signals
        ...
        for category, signals_list in category_signals.items():
            ...
            logger.info(
                f"  {category}: {len(triggered_in_category)}/{len(signals_list)} signals triggered")
            for signal in triggered_in_category:
                logger.info(
                    f"    - {signal.name}: Strength {signal.strength:.3f}, Confidence {signal.confidence:.3f}")
        ...
        # At least 2 categories should trigger
        meets_signal_threshold = 2 <= signals_count <= 5
        ...
        logger.info(
            f"  Signal Count: {signals_count} (Required: 2-5) - {'✅ PASS' if meets_signal_threshold else '❌ FAIL'}")
        logger.info(
        ...
        if should_sell:
            logger.info(
                "✅ ALL THRESHOLD CHECKS PASSED - Proceeding with sell decision")

            # Calculate stop-loss and take-profit levels
            stop_levels = self._calculate_dynamic_stops(
                position_metrics, market_context)
            ...
            base_decision = SellDecision(
                ...
                # For compatibility
                trailing_stop_price=stop_levels["active_stop"],
            ...
            final_decision = self._determine_position_sizing(
                final_decision, all_signals, position_metrics)
            ...
            if not meets_signal_threshold:
                rejection_reasons.append(
                    f"Triggered signals {signals_count} not in range [2, 5]")
    ...
    def _generate_technical_signals(self, technical: Dict, position: PositionMetrics) -> List[SellSignal]:
        ...
        if macd < macd_signal and macd < 0:
            strength = min(abs(macd - macd_signal) /
                           abs(macd_signal) if macd_signal != 0 else 1, 1.0)
            signals.append(SellSignal(
                name="macd_bearish",
                strength=strength,
    ...
    def _generate_risk_signals(self, position: PositionMetrics) -> List[SellSignal]:
        ...
        stop_loss_level = position.db_stop_loss if position.db_stop_loss is not None else position.entry_price * \
            (1 - self.base_stop_loss_pct)
        ...
        take_profit_level = position.db_target_price if position.db_target_price is not None else position.entry_price * \
            (1 + self.base_stop_loss_pct * 2)  # 2:1 risk-reward
        ...
        profit_protection_level = position.highest_price_since_entry * \
            (1 - self.profit_protection_threshold)
        # If we have a stored target price, use half of it for profit protection
        if position.db_target_price is not None:
            stored_profit_level = position.entry_price + \
                (position.db_target_price - position.entry_price) * 0.5
            profit_protection_level = max(
                profit_protection_level, stored_profit_level)
        ...
        if position.unrealized_pnl_pct < -0.05:  # 5% loss
            strength = min(abs(position.unrealized_pnl_pct) /
                           0.10, 1.0)  # Scale to 10% max loss
            signals.append(SellSignal(
                name="large_loss",
                strength=strength,
                weight=0.10,  # 10% of total weight
        ...
        if position.days_held > 30 and position.unrealized_pnl_pct < 0.02:
            # Scale to 90 days max
            strength = min(position.days_held / 90, 1.0)
            signals.append(SellSignal(
                name="time_decay",
                strength=strength,
                weight=0.07,  # 7% of total weight
    ...
    def _generate_sentiment_signals(self, sentiment: Dict) -> List[SellSignal]:
        ...
        if sentiment_score < -0.2:  # Negative sentiment threshold
            strength = min(abs(sentiment_score) / 0.8,
                           1.0)  # Scale to -0.8 max
            signals.append(SellSignal(
    ...
    def _calculate_dynamic_stops(self, position: PositionMetrics, market_context: MarketContext) -> Dict:
        """
        Calculate dynamic stop-loss and take-profit levels for active risk management.
        Industry Standard: Uses Trailing Stops and Profit Protection based on Highest Price.
        """
        curr_price = position.current_price
        entry_price = position.entry_price

        # 1. Base Stop Loss (Hard Stop)
        # Use stored value from DB if available (initial SL), otherwise calculate from entry
        base_stop = position.db_stop_loss if position.db_stop_loss is not None else entry_price * \
            (1 - self.base_stop_loss_pct)

        # 2. Volatility-Adjusted Trailing Stop
        # Multiplier increases stop distance during high volatility to avoid premature exits
        vol_adj = 1 + (position.volatility / 0.02)
        v_stop_pct = self.base_stop_loss_pct * vol_adj
        volatility_stop = position.highest_price_since_entry * (1 - v_stop_pct)

        # 3. Profit Protection (Lock-in Gains)
        profit_protection_stop = 0.0
        pnl_pct = position.unrealized_pnl_pct

        if pnl_pct > self.profit_protection_threshold:
            # Once threshold is met, we move SL to lock in 50% of the gain from peak
            peak_gain = position.highest_price_since_entry - entry_price
            lock_in_price = entry_price + (peak_gain * 0.5)
            profit_protection_stop = max(
                lock_in_price, entry_price * 1.005)  # At least break-even + 0.5%

            # If we have a stored target price, use it to tighten the stop further if 75% reached
            if position.db_target_price is not None:
                target_dist = position.db_target_price - entry_price
                if curr_price >= entry_price + (target_dist * 0.75):
                    # Very close to target: lock in 80% of gains
                    profit_protection_stop = max(
                        profit_protection_stop, entry_price + (target_dist * 0.8))

        # 4. Take Profit (Target)
        # Use DB target if set, otherwise a standard 2:1 Reward:Risk ratio
        take_profit = position.db_target_price if position.db_target_price is not None else entry_price * \
            (1 + self.base_stop_loss_pct * 2)

        # Active stop is the MOST AGGRESSIVE (highest) of available stops to protect capital
        # We always want the stop to move UP, never down.
        protective_stops = [base_stop, volatility_stop]
        if profit_protection_stop > 0:
            protective_stops.append(profit_protection_stop)

        active_stop = max(protective_stops)

        # Sanity Guard: Don't set SL above current price unless we are forcing an exit
        if active_stop >= curr_price:
            active_stop = curr_price * 0.998  # Tight trail at 0.2% if already crossed

        return {
            "base_stop": round(base_stop, 2),
            "volatility_stop": round(volatility_stop, 2),
            "profit_protection_stop": round(profit_protection_stop, 2),
    ...
    def _evaluate_signal_based_sell(
        ...
        if position.db_target_price is not None and position.current_price >= position.db_target_price:
            ...
            return SellDecision(
                ...
                # For compatibility
                trailing_stop_price=position.db_stop_loss if position.db_stop_loss is not None else 0.0,
        ...
        if should_sell:
            ...
            if not meets_signal_threshold:
                rejection_reasons.append(
                    f"Triggered categories {signals_count} not in range [2, 5]")
            if not meets_confidence_threshold:
                rejection_reasons.append(
                    f"Confidence {avg_confidence:.3f} below threshold {self.min_confidence_threshold}")
            if not meets_weighted_threshold:
    ...
    def _apply_market_context_filters(self, decision: SellDecision, market_context: MarketContext) -> SellDecision:
        ...
        if market_context.trend in [MarketTrend.STRONG_UPTREND, MarketTrend.UPTREND]:
            if decision.urgency < 0.5:  # LOWERED from 0.8 to 0.5 for less restriction
                logger.info(
                    f"SELL MODERATED: Market in {market_context.trend.value}, urgency {decision.urgency:.3f} < 0.5")
                ...
                # Use downtrend multiplier for less restriction
                decision.confidence *= self.downtrend_sell_multiplier
                logger.info(
                    f"Uptrend adjustment - confidence reduced from {original_confidence:.3f} to {decision.confidence:.3f}")
        ...
        # Be less aggressive in downtrends (allow more sells)
            ...
            # Use uptrend multiplier for less restriction
            decision.confidence *= self.uptrend_sell_multiplier
            # LESS aggressive adjustment
            decision.urgency = min(decision.urgency * 1.1, 1.0)
    ...
    def _determine_position_sizing(
        ...
        logger.info(
            f"Determining position sizing for {position.quantity} shares")
        ...
        if (decision.confidence >= self.emergency_exit_threshold or
            ...
            logger.info(
                f"Emergency exit triggered - selling 100% ({position.quantity} shares)")
        ...
        # Full exit conditions
            ...
            logger.info(
                f"Full exit triggered - selling 100% ({position.quantity} shares)")
        ...
        # Aggressive exit conditions
            ...
            # INCREASED scaling
            exit_percentage = min(decision.confidence *
                                  decision.urgency * 1.2, 0.9)
            ...
            logger.info(
                f"Aggressive exit triggered - selling {exit_percentage:.1%} ({decision.sell_quantity} shares)")
        ...
        # Partial exit conditions
            ...
            decision.sell_quantity = max(
                1, int(position.quantity * exit_percentage))
            decision.sell_percentage = exit_percentage
            decision.reasoning += f" | PARTIAL EXIT ({exit_percentage:.1%})"
            logger.info(
                f"Partial exit triggered - selling {exit_percentage:.1%} ({decision.sell_quantity} shares)")

        # Conservative exit conditions
        elif decision.confidence >= self.conservative_exit_threshold:
            # Scale exit size based on confidence and urgency
            exit_percentage = min(
                decision.confidence * decision.urgency * 0.5, 0.5)  # REDUCED scaling
            decision.sell_quantity = max(
                1, int(position.quantity * exit_percentage))
            decision.sell_percentage = exit_percentage
            decision.reasoning += f" | CONSERVATIVE EXIT ({exit_percentage:.1%})"
            logger.info(
                f"Conservative exit triggered - selling {exit_percentage:.1%} ({decision.sell_quantity} shares)")

        else:
            # Minimal exit
            decision.sell_quantity = max(
                1, int(position.quantity * 0.1))  # 10% minimum
            decision.sell_percentage = 0.1
            decision.reasoning += " | MINIMAL EXIT (10%)"
            logger.info(
                f"Minimal exit triggered - selling 10% ({decision.sell_quantity} shares)")

        # Ensure we don't sell more than we have
        decision.sell_quantity = min(decision.sell_quantity, position.quantity)
        logger.info(
            f"Final sell quantity: {decision.sell_quantity} shares ({decision.sell_percentage:.1%})")
    ...
    def _adapt_to_market_regime(self, market_context: MarketContext) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Market regime adaptation: {market_context.trend.value} -> {adapted_params}")
