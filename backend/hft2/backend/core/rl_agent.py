...
class RLFilteringAgent:
    ...
    def rank_stocks(self, universe_data: Dict[str, Any], horizon: str = "day") -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Starting RL ranking for {len(universe_data)} stocks with horizon: {horizon}")

        # Check if we have valid data
        if not universe_data:
            logger.warning("No universe data provided for RL ranking")
            return []

        # Filter out stocks with invalid/missing price data
        valid_universe = {}
        for symbol, data in universe_data.items():
            if data and isinstance(data, dict) and 'price' in data and data.get('price', 0) > 0:
                valid_universe[symbol] = data
            else:
                logger.debug(
                    f"Skipping {symbol}: invalid or missing price data")

        if len(valid_universe) == 0:
            logger.error(
                "No valid stock data available after filtering - check data source (Yahoo Finance rate limited? Try Fyers API)")
            return []

        # Log data quality warning if many stocks were filtered out
        filter_rate = 1.0 - (len(valid_universe) /
                             len(universe_data)) if len(universe_data) > 0 else 0
        if filter_rate > 0.5:
            logger.warning(
                f"⚠️ High data filtering rate: {filter_rate:.1%} of stocks had invalid data")
            logger.warning(
                f"💡 Tip: If Yahoo Finance is rate-limited, wait 1-2 hours or switch to Fyers API only")

        logger.info(
            f"Valid universe size: {len(valid_universe)} stocks (filtered from {len(universe_data)})")
        ranked_stocks = []
        ...
        for i in range(0, len(stock_items), batch_size):
            ...
            logger.debug(
                f"Processed batch {i//batch_size + 1}: {len(batch_results)} qualified stocks")
        ...
        logger.info(
            f"RL ranking completed: {len(filtered_stocks)} stocks shortlisted from {len(universe_data)}")
    ...
    def _process_batch(self, batch: List[tuple], horizon: str, risk_settings: Dict[str, float]) -> List[Dict[str, Any]]:
        ...
        for symbol, data in batch:
            if not data or 'price' not in data:
                logger.debug(f"Skipping {symbol}: missing price data")
                continue
        ...
        if not features_batch:
            logger.warning("No valid features to process in batch")
            return batch_results

        # Process batch on GPU
        try:
            features_tensor = torch.FloatTensor(
                np.array(features_batch)).to(self.device)
            ...
            # Apply risk filtering with relaxed criteria when data is scarce
            for i, (symbol, data) in enumerate(symbols_batch):
                ...
                # RELAXED Risk compliance checks - allow more stocks when data is limited
                # Original: score > 0.5 and price > 10 and price < capital_at_risk * 100
                # Relaxed: Lower score threshold and remove upper price limit constraint
                min_score_threshold = 0.3  # Reduced from 0.5
                min_price = 10  # Keep minimum price check

                is_risk_compliant = (
                    score >= min_score_threshold and  # Relaxed confidence threshold
                    price >= min_price  # Minimum price check only
                )

                # Log why stocks are not passing risk filter for debugging
                if score < min_score_threshold:
                    logger.debug(
                        f"{symbol} failed risk filter: score {score:.3f} < {min_score_threshold}")
                elif price < min_price:
                    logger.debug(
                        f"{symbol} failed risk filter: price {price} < {min_price}")
                else:
                    logger.debug(
                        f"{symbol} passed risk filter with score {score:.3f}, price {price}")
                ...
                if is_risk_compliant:
                    ...
                    logger.debug(
                        f"{symbol} did not pass risk compliance with score {score:.3f}, price {price}")

        except Exception as e:
            logger.error(f"Error processing batch: {e}")
            # Fallback to CPU processing
            logger.info("Falling back to CPU processing for batch")
            for symbol, data in symbols_batch:
                try:
                    features = self._extract_features(data, horizon)
                    score = self._get_rl_score_cpu(features)

                    # Even in fallback, apply basic risk compliance with relaxed criteria
                    price = data.get('price', 0)
                    min_score_threshold = 0.3  # Relaxed threshold
                    min_price = 10

                    if score >= min_score_threshold and price >= min_price:
                        batch_results.append({
                            "symbol": symbol,
                            "score": score,
                            "risk_compliant": True,
    ...
    def _extract_features(self, data: Dict[str, Any], horizon: str) -> np.ndarray:
        ...
        try:
            ...
            # Horizon encoding
            horizon_encoding = {"day": 1, "week": 2,
                                "month": 3, "year": 4}.get(horizon, 1)
            ...
            if not np.isfinite(features).all():
                logger.warning(
                    "Non-finite values detected in features, using defaults")
    ...
    def filter_by_horizon(self, ranked_stocks: List[Dict[str, Any]], horizon: str) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Filtered to top {len(filtered)} stocks for {horizon} horizon")
        return filtered
    ...
    def save_shortlist(self, shortlist: List[Dict[str, Any]]):
        ...
        try:
            ...
            logger.info(
                f"Saved shortlist: {len(shortlist)} stocks to logs/shortlist_{date_str}.json")
    ...
    def get_model_stats(self) -> Dict[str, Any]:
        ...
        if MONITORING_AVAILABLE:
            try:
                total_processed = self.filtering_stats.get(
                    "total_processed", 0)
