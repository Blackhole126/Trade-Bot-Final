...
# Import keyword index as fallback
try:
...
class FinanceRAGLoader:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.vectorstore_path = Path(
            config.get("vectorstore_path", "vectorstore"))
        ...
        # Initialize keyword index as fallback
        self.keyword_index = None
        if HAS_KEYWORD_INDEX and not HAS_SENTENCE_TRANSFORMERS:
            logger.info("Keyword index available as fallback")
    ...
    def _init_embedding_model(self):
        ...
        if HAS_SENTENCE_TRANSFORMERS:
            try:
                ...
                logger.warning(
                    f"Failed to load sentence-transformers model: {e}. Using fallback.")
                self.embedding_model = None
        else:
            logger.warning(
                "sentence-transformers not installed. Using fallback embedding method.")
    ...
    def _load_vectorstore(self):
        ...
        try:
            if self.embeddings_file.exists() and self.chunks_file.exists():
                ...
                logger.info(
                    f"Loaded vector store with {len(self.chunks)} chunks")
            else:
        ...
        for i in range(0, len(words), self.chunk_size - self.chunk_overlap):
            ...
            if len(chunk_words) >= 50:  # Minimum chunk size
                chunk_id = hashlib.md5(
                    f"{source}_{i}_{chunk_text}".encode()).hexdigest()[:16]
    ...
    def _generate_embedding(self, text: str) -> np.ndarray:
        ...
        if self.embedding_model is not None:
            ...
            try:
                ...
                logger.warning(
                    f"Error generating embedding with sentence-transformers: {e}")
    ...
    def _fallback_embedding(self, text: str, dim: int = 384) -> np.ndarray:
        ...
        for keyword, weight in financial_keywords.items():
            if keyword in text_lower:
                ...
                keyword_hash = int(hashlib.md5(
                    keyword.encode()).hexdigest(), 16)
    ...
    def build_vectorstore(self, kb_path: str):
        ...
        for md_file in kb_path.rglob("*.md"):
            try:
                ...
                chunks = self._chunk_text(
                    content, str(md_file.relative_to(kb_path)))
        ...
        logger.info(
            f"Built vector store: {len(all_chunks)} chunks, {self.embeddings.shape}")
    ...
    def retrieve(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        # Last resort: simple text search
            ...
            logger.warning(
                "Vector store and keyword index not available, using simple text search")
    ...
    def _retrieve_keyword(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        for result in keyword_results:
            doc_info = self.keyword_index.documents[result['doc_id']]
            results.append({
                'text': doc_info['text'],
    ...
    def _retrieve_simple(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        """Simple text matching as last resort"""
        query_terms = query.lower().split()

        # Load KB files directly
        kb_path = Path(self.config.get('kb_path', '../Finance_KB'))
        results = []
            ...
            try:
                ...
                if match_count > 0:
                    results.append({
                        ...
                        'simple_score': match_count,
                        'combined_score': float(match_count)
                    })
            except Exception:
                continue
