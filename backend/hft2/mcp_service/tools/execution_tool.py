...
from ..server.mcp_trading_server import MCPToolResult, MCPToolStatus
...
class ExecutionTool:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.trading_mode = config.get("trading_mode", "live")  # Enforce live mode
    ...
    async def _execute_live_order(self, order: TradeOrder, arguments: Dict[str, Any]) -> TradeOrder:
        ...
        try:
            ...
            logger.error("Live trading execution not fully implemented in this tool - please use the main trading bot")
            order.status = OrderStatus.REJECTED
            order.updated_at = datetime.now()
            return order
