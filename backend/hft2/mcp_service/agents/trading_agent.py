...
class TradingAgent:
    ...
    def _get_market_analyzer(self):
        ...
        if self._market_analyzer is None:
            try:
                ...
                self._market_analyzer = MarketAnalysisTool(
                    fyers_client) if fyers_client else False
            except ImportError as e:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if groq_engine:
                ...
                logger.warning(
                    "Groq engine not available - continuing without it")
            ...
            if fyers_client:
                ...
                logger.warning(
                    "Fyers client not available - continuing without it")

            # Initialize market analyzer (lazy import)
            market_analyzer = self._get_market_analyzer()
            if market_analyzer:
                self.market_analyzer = market_analyzer
            else:
                logger.warning(
                    "Market analyzer not available - continuing without it")
    ...
    - Multi-step reasoning with Groq AI
        ...
        try:
            ...
            try:
                from ..llm import TradingContext
            except ImportError:
            ...
            if getattr(self, 'groq_engine', None):
                try:
                    ...
                    # Fallback: derive simple recommendation from technicals
                    tech = technical_analysis or {}
                    if isinstance(tech, dict) and tech.get("error"):
                        rec, conf, reason = "WAIT", 0.0, f"Technical analysis unavailable: {tech.get('error', 'unknown')}. Groq error: {groq_err}"
                    else:
                        rec, conf, reason = "HOLD", 0.5, f"Groq error occurred - using technical indicators only: {groq_err}"
                    ai_decision = _SimpleAIDecision(
                        recommendation=rec, confidence=conf, reasoning=reason)
            else:
    ...
    async def _perform_technical_analysis(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            status_val = result.status.value if hasattr(
    ...
    async def _gather_market_context(self, symbol: str, external_context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not current_data:
                ...
                logger.warning(
                    f"All Fyers quotes failed for {symbol}, falling back to Yahoo Finance for current price")
                try:
                    ...
                    raise ValueError(
                        f"No market data available for {symbol}: {yf_err}")
            ...
            try:
                ...
                spread = abs(depth_data.asks[0]["price"] - depth_data.bids[0]["price"]) if (
                    depth_data and depth_data.asks and depth_data.bids) else 0
            except Exception:
                depth_info = {"bid_levels": 0, "ask_levels": 0,
        ...
        try:
            ...
            try:
                ...
                if user_port and isinstance(user_port, dict):
                    ...
                    exposure = sum(
                        (h.get("quantity") or 0) *
                        (h.get("currentPrice") or h.get("avgPrice") or 0)
    ...
    async def _get_rl_prediction(self, context: "TradingContext") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # For now, we'll simulate a prediction based on context
            portfolio_context = getattr(context, 'portfolio_data', None) or getattr(
                context, 'portfolio_context', None) or {}
    ...
    async def _assess_trade_risk(self, context: "TradingContext", ai_decision: "GroqResponse", ensemble_predictions: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # FIX: Handle None ai_decision gracefully
            ai_metadata = {}
            ...
            trade_details = {
                ...
                "market_conditions": context.market_data or {},
                "risk_parameters": getattr(context, 'risk_metrics', None) or getattr(context, 'risk_parameters', {}) or {},
                ...
                try:
                    async with self.groq_engine:
                        risk_response = await self.groq_engine.assess_trade_risk(
                            trade_details, getattr(context, 'portfolio_data', None) or getattr(
            ...
            if not risk_response:
                risk_response = type('RiskResponse', (), {
                                     'content': 'Risk assessed from technicals only (Groq not configured).', 'metadata': {'risk_factors': []}})()

            # Calculate quantitative risk metrics
            volatility = (context.market_data or {}).get("volatility", 0.02)
            ...
            return {
                ...
                "liquidity_risk": self._assess_liquidity_risk(context.market_data or {}),
    ...
    def _calculate_position_size(self, risk_assessment: Dict[str, Any], context: "TradingContext", ensemble_predictions: Dict[str, Any]) -> float:
        ...
        try:
            ...
            _portfolio = getattr(context, 'portfolio_data', None) or getattr(
                context, 'portfolio_context', None) or {}
            max_risk_amount = _portfolio.get(
    ...
    def _synthesize_trading_signal(self, symbol: str, context: "TradingContext",
        ...
        try:
            ...
            # FIX: Extract decision from AI response safely
            recommendation = "HOLD"
            ...
            volatility = (risk_assessment or {}).get("volatility", 0.02)
            ...
            combined_confidence = (ai_confidence_val * 0.7 + ml_confidence *
                                   0.3) if ml_direction != 0 else ai_confidence_val
            ...
            return TradingSignal(
                ...
                metadata={
                    "technical_signals": getattr(context, 'technical_signals', {}) if context else {},
                    "market_data": getattr(context, 'market_data', {}) if context else {},
